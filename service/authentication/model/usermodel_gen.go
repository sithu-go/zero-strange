...
type (
	...
	User struct {
		Id        int64     `db:"id"`
		Username  string    `db:"username"` // username like id
		Name      string    `db:"name"`     // Name of the user
		Password  string    `db:"password"` // User Password
		Gender    string    `db:"gender"`   // Male | Female | Disclosed
...
func (m *defaultUserModel) Insert(ctx context.Context, data *User) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Username, data.Name, data.Password, data.Gender, data.CreatedAt, data.UpdatedAt)
	}, userIdKey, userUsernameKey)
	return ret, err
}
...
func (m *defaultUserModel) Update(ctx context.Context, newData *User) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.Username, newData.Name, newData.Password, newData.Gender, newData.CreatedAt, newData.UpdatedAt, newData.Id)
	}, userIdKey, userUsernameKey)
	return err
}
